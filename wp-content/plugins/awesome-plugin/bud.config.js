...
export default async bud => {
	 ...
	 * Setting this to a unique value ('awesome-plugin') ensures that multiple plugins can operate without
	 * interfering with each other's chunk loading mechanisms, thereby avoiding runtime conflicts.
	 */
	bud.config(
		{
			output: {
				chunkLoadingGlobal: 'awesome-plugin',
			},
		}
	)
	...
	bud.entry(
		{
			"awesome-plugin-frontend": ['/frontend/js/app.js', '/frontend/scss/app.scss'],
			"awesome-plugin-admin": ['/admin/js/app.js', '/admin/scss/app.scss']
		}
	)
